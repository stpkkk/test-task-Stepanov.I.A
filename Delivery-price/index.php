...
// абстрактный класс для всех перевозчиков, функция для расчета стоимости доставки посылки

abstract class Carrier {
	abstract public function calculateShippingCost($weight);
}

// класс для перевозчика "Почта России"
...
// класс, который будет использоваться для расчета стоимости доставки посылки для любого из перевозчиков

class ShippingCalculator {
	public function calculate($weight, Carrier $carrier) {
...
$russianPostCost = $calculator->calculate($weight, $russianPost);
$dhlCost = $calculator->calculate($weight, $dhl);

echo "Russian Post cost: $russianPostCost";
echo "DHL cost: $dhlCost";
