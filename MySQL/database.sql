...
-- 2)
 SELECT products.*, catalogs.name AS catalog_name FROM products, catalogs WHERE catalogs.id = products.catalog_id;
...
-- 4)
SELECT * FROM users 
WHERE birthday_at < CURDATE() - INTERVAL 30 YEAR 
AND id in (SELECT user_id FROM orders GROUP BY user_id HAVING COUNT(*) > 2);